...
exports.validateTriggerRequest =
  exports.initiateSearch =
  exports.initiateCall =
  exports.getResultsFromApp =
  exports.addResultToApp =
...
const getAckResponse = (req) =>
  __awaiter(void 0, void 0, void 0, function* () {
    const res_data = yield promises_1.default.readFile(
      `./mock_json_files/ack.json`,
      "utf-8"
    );
...
const getAllUseCases = () =>
  __awaiter(void 0, void 0, void 0, function* () {
    const dir = yield promises_1.default.readdir("./mock_json_files", {
      withFileTypes: true,
    });
    const domains = dir
      .filter((d) => d.isDirectory() && d.name !== "generic_jsons")
      .map((d) => d.name);
    let use_cases = {};
    for (let index = 0; index < domains.length; index++) {
      const domain = domains[index];
      const dir = yield promises_1.default.readdir(
        "./mock_json_files/" + domain,
        { withFileTypes: true }
      );
...
const isValidUseCase = (domain, use_case) =>
  __awaiter(void 0, void 0, void 0, function* () {
...
exports.wait = wait;
const getMockResponse = (domain, use_case, api) =>
  __awaiter(void 0, void 0, void 0, function* () {
    const use_case_api = use_case.split("/")[0];
    const file_path =
      use_case_api !== api ? `generic_jsons/${api}` : `${domain}/${use_case}`;
    const req_data = yield promises_1.default.readFile(
      `./mock_json_files/${file_path}.json`,
      "utf-8"
    );
...
const initiateCall = (req, api) =>
  __awaiter(void 0, void 0, void 0, function* () {
    try {
      ...
      console.log(
        `${config_1.default.get("bap_id")} : ${
          request.context.transaction_id
        }  Sending ${api} to ${end_point}`
      );
      const res = yield axios_1.default.post(end_point, request, {
        headers: { use_case },
      });
    } catch (error) {
...
const initiateSearch = (req) =>
  __awaiter(void 0, void 0, void 0, function* () {
    try {
      ...
      const end_point = bpp_uri
        ? combineURLs(bpp_uri, "/search")
        : combineURLs(bg_uri, "/search");
      console.log(bpp_uri, end_point);
      const search_request = yield (0, exports.getMockResponse)(
        domain,
        use_case,
        "search"
      );
      ...
      console.log(
        `${config_1.default.get("bap_id")} : ${
          search_request.context.transaction_id
        }  Sending search to ${end_point}`
      );
      const res = yield axios_1.default.post(end_point, search_request, {
        headers: { use_case },
      });
...
const validateTriggerRequest = (req, api) =>
  __awaiter(void 0, void 0, void 0, function* () {
    const allowedApis = [
      "search",
      "select",
    ...
    if (!allowedApis.includes(api)) {
      console.log(
        `${config_1.default.get("bap_id")} ERROR : Invalid api ${api}`
      );
      return {
        message: `Only the following apis are allowed to be triggered:${allowedApis.toString()}`,
      };
    }
    ...
    if (
      ...
      console.log(
        `${config_1.default.get(
          "bap_id"
        )} ERROR : Invalid inputs domain and usecase required`
      );
      const all_use_cases = yield (0, exports.getAllUseCases)();
      return {
        message: `domain and use_case are required inputs`,
        supported_use_cases_and_domains: all_use_cases,
      };
    }
    if (api !== "search") {
      if (bpp_uri === undefined || bpp_uri === "") {
        console.log(
          `${config_1.default.get(
            "bap_id"
          )} ERROR : Invalid inputs bpp_uri required`
        );
      ...
      if (transaction_id === undefined || transaction_id === "") {
        console.log(
          `${config_1.default.get(
            "bap_id"
          )} ERROR : Invalid inputs transaction_id required`
        );
        return { message: `transaction_id is a required input` };
      }
    }
    const valid_use_case = yield (0, exports.isValidUseCase)(
      req.body.domain,
      req.body.use_case
    );
    if (!valid_use_case) {
      ...
      console.log(
        `${config_1.default.get("bap_id")} ERROR : Invalid domain/use case`
      );
      return {
    ...
    if (!use_case.split("/")[0].includes(api)) {
      return {
        message: `The specified use case (${
          use_case.split("/")[0]
        }) and triggered api (${api}) do not match`,
      };
    }
    return { message: "" };
  });
